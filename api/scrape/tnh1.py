...
class TNH1Scraper(BaseScraper):
    ...
    def get_article_links(self, page) -> list[str]:
        ...
        for a in page.css("a[href]"):
            ...
            if href.startswith("https://tnh1.com.br/") and len(href) > 25:
                if href not in links:
                    links.append(href)
        return links[:20]

    def parse_article(self, page, url: str) -> Article | None:
        title_el = page.css_first("h1.entry-title") or page.css_first("h1") or page.css_first(".titulo")
        if title_el is None:
            return None
        title = title_el.text.strip()

        body_el = page.css_first(".entry-content") or page.css_first(".texto-noticia") or page.css_first("article")
