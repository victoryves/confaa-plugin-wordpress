...
class GazetaWebScraper(BaseScraper):
    ...
    listing_url = "https://gazetaweb.com/"

    def get_article_links(self, page) -> list[str]:
        links = []
        for a in page.css("a[href]"):
            href = a.attrib.get("href", "")
            if href.startswith("https://gazetaweb.com/") and len(href) > 25:
                if href not in links:
                    links.append(href)
        return links[:20]

    def parse_article(self, page, url: str) -> Article | None:
        title_el = page.css_first("h1") or page.css_first(".entry-title")
        if title_el is None:
            return None
        title = title_el.text.strip()

        body_el = page.css_first(".entry-content") or page.css_first(".post-content") or page.css_first("article")
