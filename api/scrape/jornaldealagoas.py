...
class JornalDeAlagoasScraper(BaseScraper):
    ...
    def get_article_links(self, soup: BeautifulSoup) -> list[str]:
        ...
        for a in soup.select("a[href]"):
            href = a.get("href", "")
            if isinstance(href, str) and href.startswith("https://jornaldealagoas.com.br/") and len(href) > 35:
    ...
    def parse_article(self, soup: BeautifulSoup, url: str) -> Article | None:
        ...
        body_el = soup.select_one(".entry-content") or soup.select_one(".post-body") or soup.select_one("article")
